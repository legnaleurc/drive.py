...
async def main(args: list[str] = None):
    ...
    async with src_factory() as src_drive, dst_factory() as dst_drive:
...
async def migrate_folder(
    src_drive: Drive, src_node: Node, dst_drive: Drive, dst_root: Node
):
...
async def migrate_file(
    src_drive: Drive, src_node: Node, dst_drive: Drive, dst_root: Node, quota: list[int]
):
    new_node = await dst_drive.get_node_by_name_from_parent(src_node.name, dst_root)
    if new_node:
...
async def locked_migrate_folder(
...
async def copy_node(src_drive: Drive, src_node: Node, dst_drive: Drive, new_root: Node):
    ...
    if src_node.is_video:
        media_info = MediaInfo.video(
            src_node.video_width, src_node.video_height, src_node.video_ms_duration
        )
...
def get_migrated_size() -> int:
    ...
    with migration_cache() as query:
        query.execute(
            "SELECT SUM(size) FROM migrated WHERE created_at >= ?;", (yesterday,)
        )
        row = query.fetchone()
        if not row:
            return 0
